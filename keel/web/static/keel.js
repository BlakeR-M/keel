/* Keel web: the chat form posts by fetch and swaps the result partial in place, so the page stays
   put while the model works. Without JavaScript the same form posts normally and the server renders
   the whole page with the result in it. Ctrl+Enter (Cmd+Enter on a Mac) in the question box sends
   the form; a small elapsed counter keeps the wait honest on a CPU model. On the hosted demo a
   compare button asks the restricted question as both demo users, side by side, through the same
   /ask path a typed question takes. */
(function () {
  "use strict";
  ...
  function errorMarkup() {
    return '<article class="result error"><header class="result-head"><span class="tag state-error">error</span></header>' +
      '<p class="answer-text">The request stopped before it reached the server. Check that Keel is running, then send the question again.</p></article>';
  }
  ...
  /* Hosted demo only: the intro dialog shows once per browser and stays reachable from the banner
     link. The seen flag is written at show time, so any way of dismissing it counts. With storage
     blocked it stays closed rather than greeting the visitor on every load. */
  ...
  /* Hosted demo only: one click asks the restricted question as both demo users. Each column is a
     real request through the same engine, so the refusal and the cited answer land as they run. */
  var compare = document.getElementById("demo-compare");
  var compareSection = document.getElementById("compare");
  if (compare && compareSection && window.fetch && window.FormData) {
    compare.addEventListener("click", function () {
      compare.disabled = true;
      compareSection.innerHTML =
        '<div class="compare-grid">' +
        '<div class="compare-col"><h2 class="compare-head">public</h2><div class="compare-result"></div></div>' +
        '<div class="compare-col"><h2 class="compare-head">hr-officer</h2><div class="compare-result"></div></div>' +
        '</div>';
