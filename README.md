...
The full suite is 603 tests across 19 files (`.venv\Scripts\python.exe -m pytest --collect-only -q`),
...
| [`tests/`](tests/conftest.py) | 603 tests including the three `redteam_*.py` files; [`tests/fakes.py`](tests/fakes.py) holds the `FakeLLM` the unit tests use |
