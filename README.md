...
- **Hosted demo on Railway** ([`deploy/railway/README.md`](deploy/railway/README.md)): two services,
  ...
  It runs the fixture corpus with `KEEL_DEMO_IDENTITY=1`, which honours the chat page's user picker
  beyond loopback so a visitor can compare `public` and `hr-officer`; that flag is for the hosted
  demo of the fixture corpus and is never set for a real deployment ([`docs/web.md`](docs/web.md)).
...
| [`tests/`](tests/conftest.py) | 585 tests including the three `redteam_*.py` files; [`tests/fakes.py`](tests/fakes.py) holds the `FakeLLM` the unit tests use |
