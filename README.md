...
The full suite is 679 tests across 21 files (`.venv\Scripts\python.exe -m pytest --collect-only -q`),
...
| [`tests/`](tests/conftest.py) | 679 tests including the three `redteam_*.py` files; [`tests/fakes.py`](tests/fakes.py) holds the `FakeLLM` the unit tests use |
