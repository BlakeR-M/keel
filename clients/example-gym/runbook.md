# Example Gym: Keel runbook

One page for the person who installs and runs the appliance at the gym. Keel runs on one Windows
machine at Example Gym (the front-desk PC is the natural home), air-gapped: the model, the documents and
every log stay on that machine.

Files in this overlay: `keel.yaml` (settings, roles, users), `policy.yaml` (tool policy),
`corpus.manifest.yaml` (ingest template), `needs-from-client.md` (what the gym provides first).
...
3. Set the overlay's environment. Values come from `keel.yaml` `settings`; set them for the Windows
   ...
   [Environment]::SetEnvironmentVariable('KEEL_DATA_DIR',     'C:\Keel\example-gym\data',           'User')
...
4. Create the data directory and install the tool policy. `keel/providers/factory.py` loads the policy
   ...
   ```powershell
   New-Item -ItemType Directory -Force C:\Keel\example-gym\data, C:\Keel\example-gym\corpus | Out-Null
   Copy-Item clients\example-gym\policy.yaml C:\Keel\example-gym\data\policy.yaml
   Copy-Item clients\example-gym\corpus.manifest.yaml C:\Keel\example-gym\corpus.manifest.yaml
   ```
...
5. Start the model server, ingest (section 3), then start the web app:
   ...
   .venv\Scripts\python.exe -m keel.cli ingest --manifest C:\Keel\example-gym\corpus.manifest.yaml
   .\deploy\onprem\run.ps1
   ```

   The app answers at http://127.0.0.1:8400 on that machine only (bound to loopback). Logs and pid files
   sit in `C:\Keel\example-gym\data\`. Stop everything with `.\deploy\onprem\stop.ps1`.
6. Verify three things before handing over: a trainer user asking about refunds gets a refusal (the
   front-desk tag is outside their role); the owner user gets an answer with citations;
...
Documents live outside the repository, under `C:\Keel\example-gym\corpus\<class>\`. The manifest copy from
step 4 lists one entry per file with its `acl_tags`. Tag with the narrowest role that should read the
document; wider roles carry the narrower tags too (`keel.yaml` `roles`).

| Document class | Folder | Tag | Who can retrieve it |
| --- | --- | --- | --- |
| Membership terms, class schedule, gym rules | `corpus\public\` | `public` | everyone using the appliance |
| Staff procedures, emergency and first aid plan, code of conduct | `corpus\staff\` | `staff` | front desk, trainers, owner |
| Opening and closing, point of sale and refunds, blank waiver, waiver handling | `corpus\front-desk\` | `front-desk` | front desk, owner |
| Programming guide, class run sheets, equipment checks | `corpus\trainers\` | `trainer` | trainers, owner |
| Insurance, supplier contracts | `corpus\management\` | `owner` | owner |
...
(below) so answers cite the current version. The class schedule changes each term, so plan for that
rhythm.
...
```powershell
.venv\Scripts\python.exe -c "import sqlite3; c=sqlite3.connect(r'C:\Keel\example-gym\data\keel.db'); c.execute('PRAGMA foreign_keys=ON'); n=c.execute('DELETE FROM documents WHERE source LIKE ?', ('%class-schedule.pdf',)).rowcount; c.commit(); print(n, 'document(s) removed')"
```

Signed waivers hold member details. The pilot ingests the blank waiver and the handling procedure only,
tagged `front-desk`; the owner decides in `needs-from-client.md` whether member records ever join the
corpus, and if they do, they are tagged `front-desk` and redacted first with the PII redactor
...
this overlay, one entry per user with the tags of their role, so a trainer's question is answered from
trainer, staff and public material only.
...
- Placeholder ids (`trainer-1`, `front-desk-1`) are replaced with the gym's own ids on their machine.
  Real names stay out of this repository.
- Per-person login is a later release; the pilot's guard is that the app answers on loopback only, on
  a machine the gym controls.
...
Everything Keel knows sits in one file, `C:\Keel\example-gym\data\keel.db` (with `-wal` and `-shm`
companions while the app runs). Hot backup while the app is running:

```powershell
New-Item -ItemType Directory -Force C:\Keel\example-gym\backups | Out-Null
.venv\Scripts\python.exe -c "import sqlite3, datetime; s=sqlite3.connect(r'C:\Keel\example-gym\data\keel.db'); d=sqlite3.connect(r'C:\Keel\example-gym\backups\keel-%s.db' % datetime.date.today()); s.backup(d); d.close(); print('backup written')"
```
...
- llama-server exits at start: read `C:\Keel\example-gym\data\llama-server.err`; the usual causes are a wrong
  `KEEL_MODEL_PATH` or too little RAM for the model. `-TimeoutSeconds 600` helps a slow disk.
