...
| The machine and its login | The client's PC (pilots) or the Container App | Everything above sits behind it |
...
1. **Documents in.** Files cross from "content someone wrote" to "chunks the model may see". Controls
   ...
   at load.
2. **Users to requests.** A person becomes a `User(user_id, tags)`. In the pilot build identity is
...
| T3 | The model calls a tool outside the deployment's intent, or with dangerous arguments | Per-deployment allowlist, argument rules, tool-call budget; registry schema validation; calculator evaluates numbers and operators only; `sql_readonly` is SELECT-only over allowlisted tables with a read-only authoriser, row cap and step budget; `http_get` needs an allowlisted host and is off under air-gap | `keel/agent/policy.py`, `keel/agent/tools.py`, `keel/agent/loop.py` | `tests/test_policy.py` (disallowed tool refused and logged; SQL table allowlist; DDL and multi-statement refused; host allowlist; air-gap refusal), `tests/test_agent.py` (unknown tool refused and the loop continues; budget enforced), `tests/test_clients.py` (both client policies load and decide as intended) |
...
| T7 | Personal information in the corpus reaches a user or the log | Narrow tags for documents that hold it; deterministic PII detection and redaction with check-digit validation as a pass before ingest; per-client rules in the runbooks | `keel/safety/pii.py`, `clients/*/runbook.md`, `clients/*/corpus.manifest.yaml` | `tests/test_safety.py` (TFN, Medicare, ABN, card, phone, email detection and redaction), `tests/test_clients.py` (client files carry no personal identifiers; manifests tag every document) |
...
| T12 | The tool policy is loosened without anyone noticing | Policy is one file in the data directory; client policies are versioned in `clients/` and tested; refusals are logged | `keel/providers/factory.py` (`_load_policy`), `clients/*/policy.yaml` | `tests/test_clients.py` (search_docs allowed, create_ticket queued, http_get denied, SQL allowlist, empty host list refuses every host) |

## Residual risks

- **Self-asserted identity in the pilot build.** A person at the machine can pick another user in the
...
| Tool policy and tool guards (T3, T12) | `python -m pytest tests/test_policy.py tests/test_clients.py -q` |
...
| PII redaction and clean client files (T7) | `python -m pytest tests/test_safety.py -q -k "tfn or email or card or abn or detect"` and `python -m pytest tests/test_clients.py -q -k identifiers` |
