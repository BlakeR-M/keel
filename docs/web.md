...
| `POST /admin/approvals/{approval_id}/approve` | `ctx.approvals.decide(id, True, by)` then `execute` through the registry; the result lands on the row. Optional form field `by` (default `admin`). Redirects to `/admin#approvals`, or JSON with `Accept: application/json`. A row that is no longer pending answers 409. |
| `POST /admin/approvals/{approval_id}/reject` | `decide(id, False, by)`. Nothing runs. |
| `POST /admin/ingest` | Multipart upload of one document: `file`, optional `tags` (comma separated, `public` when empty), optional `title`, optional `by`. The name is reduced to its final component and checked against the loaders' own suffix table, the size cap is enforced while the stream is read, and the file is staged in a directory of its own that is removed either way. From there it is an ordinary `ingest_path`: the same chunking, the same injection screen, the same ledger row inside the same transaction. **This route is registered only where `KEEL_DEMO_READONLY` is off.** On the hosted demo of the fixture corpus it is absent from the route table rather than present and refusing. |
| `GET /admin/connection` | What Keel is pointed at and whether it answers: the configured endpoint and model, the `keel doctor` checks, and the model servers answering on this machine. Probing costs a couple of seconds, which is why it sits on its own page rather than on every admin load. |
| `POST /admin/connection` | Body `{base_url, model}`. Switches the model, updating the running providers in place so the change applies to the next question, and writing `.env` so it survives a restart. **Loopback only.** An endpoint elsewhere is refused, because a form accepting any address lets whoever reaches it read every question and every retrieved passage by pointing the model at a host of their own. An off-machine endpoint, an unreachable one, or a model that endpoint does not serve all get 400. Registered only where `KEEL_DEMO_READONLY` is off. |
| `POST /admin/documents/{document_id}/retag` | Body `{tags, by}`. Replaces the document's access tags and its chunks' tags together, inside one transaction with a ledger row. An empty `tags` becomes `public`. 404 when the document is absent. Registered only where `KEEL_DEMO_READONLY` is off. |
| `POST /admin/documents/{document_id}/remove` | Body `{by}`. Takes the document out with its chunks, its full-text entries and its embeddings. The ledger row is written before the rows go, inside the same transaction. 404 when the document is absent. Registered only where `KEEL_DEMO_READONLY` is off. |
