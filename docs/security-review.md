...
| A1 | Beyond loopback, self-asserted `tags` in the body or query string widened access; no trusted identity channel | high | fixed: `trusted_identity` in `keel/web/app.py`; beyond loopback identity comes only from `X-Keel-User`/`X-Keel-Tags` with `X-Keel-Proxy-Token` = `KEEL_PROXY_TOKEN`, everyone else runs as `public` | `redteam_acl...::test_a11_source_viewer_query_string_tags_cannot_widen_beyond_loopback`, `::test_a12_api_ask_body_tags_cannot_widen_beyond_loopback` |
...
| A4 | `SqliteVectorIndex` served a stale ACL mask after a tag update | low | fixed: fingerprint covers tags and quarantine ids; returned rows are re-checked against fresh data and the cache reloads when stale | `redteam_acl...::test_a19_vector_index_honours_tag_narrowing_without_explicit_invalidate`; `redteam_ledger...::test_vector_index_excludes_a_chunk_quarantined_after_its_cache_warmed` |
| A5 | Malformed `acl_tags` JSON broke BM25 for everyone | low | fixed: `json_valid`/`json_type` guard, such rows are visible to nobody | `redteam_acl...::test_a05_malformed_stored_acl_json_must_not_break_retrieval_for_everyone` |
| A6 | Oversized tag list crashed BM25 (`too many SQL variables`) | low | fixed: tags bound as one JSON array | `redteam_acl...::test_a20_oversized_tag_list_must_not_crash_retrieval` |
| A7 | No ACL re-check at the generation boundary | low | fixed: `AnswerEngine.answer` and `search_docs` re-check `allowed(hit, user.tags)` | `redteam_acl...::test_a21_generation_boundary_rechecks_acl_against_a_leaky_retriever` |
